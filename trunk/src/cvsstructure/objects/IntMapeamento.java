 ...
 * @author andrein
 */
public class IntMapeamento implements Runnable {
    ...
    private static PreparedStatement psVerifyObjectType = null;
    private static ResultSet rsVerifyObjectType = null;
    ...
    public void run() {
        ...
        StringBuilder sbVerifyObjectType = new StringBuilder();
        sbVerifyObjectType.append("select * from user_objects where object_name = ?");
        ...
        try {
            if (PrepararConsultas.existObject("INT_MAPEAMENTO_LAYOUT", system) >= 1) {
                ...
                if (system.toUpperCase().equals("INOUT")) {
                    psVerifyObjectType = ConnectionInout.getConnection().prepareStatement(sbVerifyObjectType.toString());
                } else{
                    psVerifyObjectType = ConnectionIntegracao.getConnection().prepareStatement(sbVerifyObjectType.toString());
                }
                psVerifyObjectType.setString(1, "INT_MAPEAMENTO_LAYOUT");
                rsVerifyObjectType = psVerifyObjectType.executeQuery();
                rsVerifyObjectType.next();
                if(rsVerifyObjectType.getString("OBJECT_TYPE").equals("TABLE") ){
